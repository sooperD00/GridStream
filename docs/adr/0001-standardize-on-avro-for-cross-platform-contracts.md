...
## Context
Following the integration of three legacy energy-tech platforms, data is being produced by globally distributed teams in varying formats (JSON, Protobuf, and raw CSV). Inconsistent schemas are causing downstream consumer failures, and JSON payloads are incurring significant overhead at SCADA scale (millions of events per minute).
...
## Consequences
*   **Positive:** Enforces strict schema evolution (backward/forward compatibility), ensuring an EU-based SCADA team doesn't break the US Analytics team's pipelines.
